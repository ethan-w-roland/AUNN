...
class AUNN(nn.Module):
    def __init__(self, config: Config):
        ...
        self.mem = nn.Sequential(*(Block(config) for _ in range(config.n_layer)))
        self.wRNN = wRNN(config)
    ...
    def forward(
        ...
        x1 = self.proj(x1)
        x1 = self.mem(x1)
        x2 = x1 + self.wRNN.blocks(x1)
        x2 = self.wRNN.norm(x2)
        logits = self.wRNN.out_emb(x2) #(B, T, E) -> (B, T, V)
